...
export const LessonsList = ({ isAuth, teachers, church, onEdit, lessons }) => {
  ...
  const handleRemoveTeacher = useCallback(
    async (id) => {
      ...
      await editTeacher({
        ...editableTeacher,
        church: editableTeacher?.church?.filter(
  ...
  const handleAddTeachers = useCallback(async () => {
    const newData = {
      ...church,
      teachers: [...teachers?.map((el) => el.id), ...teachersList],
    };
    await editEntity(newData);
          ...
          {teachers &&
            ...
            teachers?.map((el) => (
              <ShadowCardStyled key={el.id} className="vertical-card">
                ...
                <ul>
                  <li>
                    <b>Students:</b> {el.students?.length}
                  </li>
                  <li>
                    <b>Groups:</b> {el.groups?.length}
                  </li>
                  <li>
                    <b>Created lessons:</b> {el.lessons?.length}
                  </li>
                  <li>
                    <b>Created scenarios:</b> {el.scenarios?.length}
                  </li>
                </ul>
                {isAuth && (
