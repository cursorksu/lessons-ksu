...
export const EntityItemExpanded = ({
  ...
  const { removeEntityFromArrayField, addEntityToArrayField } = useAssignEntityToLesson(entityName);
  ...
  const assignToLesson = async () => {
    try {
      ...
      await editEntity({id: item.id, lessons: [...item.lessons, selectedLesson]});
      setSelectedLesson('');
      onConfirm();
    } catch (err) {
      throw new Error(err);
